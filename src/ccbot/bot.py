...
#
# These are loaded from ~/.ccbot/skills.json if it exists (generated by
# ccbot-sync-skills script), with hardcoded defaults as fallback.
# The skills.json file is NOT touched by upstream ccbot updates or rulesync.
_DEFAULT_SKILL_COMMANDS: dict[str, tuple[str, str]] = {
    # GSD workflow
...
def _load_skill_commands() -> dict[str, tuple[str, str]]:
    """Load skill commands: hardcoded defaults merged with ~/.ccbot/skills.json.

    skills.json format: {"tg_name": {"command": "/gsd:progress", "description": "..."}}
    Entries from skills.json override defaults with the same key, and new entries
    are added. This way hardcoded commands (like beads, which aren't in
    .claude/commands/) survive, while the sync script can add/update others.
    """
    result = dict(_DEFAULT_SKILL_COMMANDS)

    skills_file = config.config_dir / "skills.json"
    if not skills_file.exists():
        logger.debug("No skills.json found, using %d hardcoded defaults",
                      len(result))
        return result
    ...
    try:
        ...
        if not isinstance(data, dict):
            logger.warning("skills.json: expected dict, got %s — using defaults only",
                           type(data).__name__)
            return result

        loaded = 0
        for tg_name, info in data.items():
            if isinstance(info, dict):
                cmd = info.get("command", "")
                desc = info.get("description", "")
                if cmd:
                    result[tg_name] = (cmd, f"↗ {desc}" if desc else "↗ Skill command")
                    loaded += 1
        logger.info("Merged %d skill commands from %s (total: %d)",
                    loaded, skills_file, len(result))
        return result
    except Exception as e:
        logger.warning("Failed to load skills.json: %s — using defaults only", e)
        return result


SKILL_COMMANDS: dict[str, tuple[str, str]] = _load_skill_commands()
